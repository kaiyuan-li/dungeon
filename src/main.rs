...
// This looks like circular dependency, prelude uses map, map uses prelude.
use prelude::*;
...
impl GameState for State {
    fn tick(&mut self, ctx: &mut BTerm) {
        ...
        ctx.cls();
        // there can only be one resource of the same type, so old keys are overwritten.
        self.resources.insert(ctx.key);
        let current_state = self.resources.get::<TurnState>().unwrap().clone();
        let systems = match current_state {
